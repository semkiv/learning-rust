...
fn integers() {
    ...
    let bin_representation = 0b0000_0000_0110_0001; // prefix `0b` signifies binary notation; optional separator `_` is allowed for numeric literals
...
fn tuples() {
    let transparent_navy: (u8, u8, u8, f64) = (0, 0, 128, 0.5); // should normally be a `struct`, tuple only as an example; alternative form would be `let transparent_navy = (0u8, 0u8, 128u8, 0.5)`; without explicit specification type would be inferred to `(i32, i32, i32, f64)`
